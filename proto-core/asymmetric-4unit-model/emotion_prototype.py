...
class EmotionVGE:
    def __init__(self, seed=42):
        ...
        self.emotional_bias = 0.0          # -1.0〜+1.0（+がポジティブ寄り）
        self.desired_tension = 0.5         # LLMが指示するテンション目標値
        self.history = []

    def set_emotional_bias(self, bias: float):
        """LLMから情緒バイアスを受け取る（+1寄り or -1寄り）"""
        self.emotional_bias = np.clip(bias, -1.0, 1.0)

    def set_desired_tension(self, tension: float):
        """LLMが主に操作するパラメータ：テンション目標"""
        self.desired_tension = np.clip(tension, 0.0, 2.0)

    def inject_over_input(self, strength: float = 1.0, polarity: float = 0.0):
        """
        オーバー入力をシミュレート。
        strength: 入力の強さ（加速圧）
        polarity: +1寄りか-1寄りか（情緒的な方向）
        """
        bias_effect = polarity * strength * 0.4
        for v in self.vge.layer_plus.voids:
            v.pos += np.array([bias_effect, bias_effect * 0.5]) * strength
        for v in self.vge.layer_minus.voids:
            v.pos += np.array([-bias_effect * 0.7, bias_effect * 0.3]) * strength

    def step(self, over_input_strength=0.0, over_input_polarity=0.0):
        if over_input_strength > 0:
            self.inject_over_input(over_input_strength, over_input_polarity)
        ...
        text = self._generate_emotional_text(emotion_balance, tension, assist)
    ...
    def _generate_emotional_text(self, balance: float, tension: float, assist: float) -> str:
        if tension > 1.2:
            if assist > 0.3:
    ...
    def run_demo(self, steps=30):
        print("=== VGE4 情緒プロトタイプ デモ ===\n")
        print("LLMは主に『テンション』を操作するだけで情緒表現が変わることを確認\n")

        for t in range(steps):
            if t < 8:
                self.set_desired_tension(0.4)
                over_str, over_pol = 0.3, 0.6
            elif t < 15:
                self.set_desired_tension(1.1)
                over_str, over_pol = 1.2, 0.8
            elif t < 22:
                self.set_desired_tension(0.9)
                over_str, over_pol = 0.8, -0.5
            else:
                self.set_desired_tension(0.6)
                over_str, over_pol = 0.4, 0.0
            ...
            print(f"t={t:2d} | テンション={state['tension']:.2f} | アシスト={state['assist']:.2f} | "
                  f"情緒バランス={state['emotion_balance']:+.2f}")
            print(f"     → {state['text']}\n")

        print("=== デモ終了 ===")
        print("テンションを上げると相反運動が活発になり、冷却アシストが自動で入って情緒が整えられる構造が確認できる。")


if __name__ == "__main__":
    demo = EmotionVGE(seed=777)
    demo.run_demo(28)
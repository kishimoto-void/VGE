...
class VGE4_With_Layer0:
    def __init__(self, seed=42):
        ...
        self.layer0_attraction_history = []
        self.history = []
    ...
    def step(self, over_input_strength=0.0, over_input_polarity=0.0):
        ...
        if over_input_strength > 0:
            ...
            bias_eff = over_input_polarity * effective * 0.45
        ...
        self.layer0_attraction_history.append(np.linalg.norm(attraction))
        ...
        text = self._generate_text(emotion_balance, tension, assist, rapid_cooling)

        record = {
            "tension": round(tension, 3),
            "assist": round(assist, 3),
            "emotion_balance": round(emotion_balance, 3),
            "layer0_attraction": round(np.linalg.norm(attraction), 4),
    ...
    def _generate_text(self, balance, tension, assist, rapid_cooling):
        if rapid_cooling:
            base = "急に冷やされて、全体が少し引き締められた"
        elif tension > 1.0:
            base = "熱がこみ上げてくるつつも、どこかで全体が纏がりを保ようとしている"
        elif tension > 0.6:
        ...
        return f"{base}。{mood}（テンション{tension:.1f} / Layer0引力{np.mean(self.layer0_attraction_history[-5:]):.3f}）"

    def run_demo(self, steps=30):
        print("=== VGE4 + Layer 0 プロトタイプ ===\n")
        print("Layer 0 が +1 と -1 の差分を引力に変換して均衡を維持する\n")
        ...
        for t in range(steps):
            ...
            print(f"t={t:2d} | テンション={state['tension']:.2f} | アシスト={state['assist']:.2f} | "
                  f"Layer0引力={state['layer0_attraction']:.4f}")
            print(f"     → {state['text']}\n")

        print("=== デモ終了 ===")
        print(f"最終维持コスト: {self.tension_maintenance_cost:.1f}")
        print(f"Layer0平均引力: {np.mean(self.layer0_attraction_history):.4f}")


if __name__ == "__main__":
    demo = VGE4_With_Layer0(seed=777)
    demo.run_demo(28)